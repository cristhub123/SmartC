...
const ROADMAP = [
  ...
  { id:'r32', status:'idea', priority:'low', title:'Event tracking avanzado (Google Analytics) + mapa de calor',
    desc:'Cita textual del usuario (documento pegado, sin modificar salvo formato): "Implementar un sistema de seguimiento de clics (event tracking)... Puntos de Interacción Críticos: Marcadores de Lugares en el Mapa (qué edificios atraen más interés); El Cluster o Grupo de Lugares (cuánta gente hace clic para expandir el grupo); Filtros Inferiores (qué categorías son más demandadas); Menú de Navegá por Zona (qué barrios consultan más). ¿Qué métricas obtener? Tasa de Conversión de Clic (cuántos ven un marcador y hacen clic); Frecuencia por Usuario (si vuelven a los mismos lugares o exploran zonas nuevas); Clics fallidos (zonas donde hacen clic pero no hay nada). Recomendación estratégica: implementar un Mapa de Calor (Heatmap) básico vinculado a los marcadores, para ver visualmente qué zonas están calientes o frías." — Análisis: los 4 puntos de interacción críticos (pin, cluster, filtros, zonas) son viables como eventos de Analytics, ~2-3 hs en total. "Frecuencia por usuario" ya lo da Analytics de fábrica, sin trabajo extra. "Tasa de conversión" (vistas vs. clicks) es conceptualmente difícil de definir bien en un mapa (a diferencia de un feed, casi todos los pines visibles se "ven" a la vez al cargar) — bajo valor para el esfuerzo, se descarta por ahora. "Clics fallidos" es posible pero de señal ruidosa (tocar el mapa vacío suele ser para cerrar un panel, no frustración) — baja prioridad. IMPORTANTE: el mapa de calor NO se resuelve con Google Analytics (no tiene soporte para coordenadas geográficas propias) — se resuelve con los datos de clicks que YA se guardan en Firestore + una librería tipo Leaflet.heat, pintando el calor directo sobre el mapa propio de la app, en vivo, sin depender de reportes con demora de Analytics. Estimado heatmap: ~2-3 hs.' },
  { id:'r33', status:'idea', priority:'mid', title:'Auditoría de consolidación de lógica repetida',
    desc:'Recorrer la app completa buscando el mismo mal planteamiento que tenía la visibilidad de pines antes de unificarse (PLAN_VISIBILIDAD_PINES_UNIFICADA.md): lógica repetida en varios archivos sin enterarse entre sí. Evaluar, caso por caso, si conviene establecerla en 1 solo lugar — solo cuando sea pertinente, priorizando buen funcionamiento, evitar errores por funciones repetidas, simplicidad/moldeabilidad para features nuevas, y menos puntos de contacto donde se chequea la misma información. Plan de la auditoría: ver PLAN_AUDITORIA_CONSOLIDACION.md.' },
  { id:'r34', status:'idea', priority:'mid', title:'Tamaño de pin en mapa y maximizado como % del espacio libre de pantalla (reemplaza breakpoint fijo mobile/desktop)',
    desc:'[2026-09-17] Detalle completo y estado en PLAN_TAMANO_PIN_ADAPTATIVO.md. Evaluación de Claude (registrada para retomar a futuro): la solución rápida ya implementada (sliders separados mobile/desktop con matchMedia de 768px) es segura para el MVP porque es aditiva y no toca los valores que ya funcionan en mobile, pero es un breakpoint fijo — no se adapta solo a resoluciones nuevas ni a formas de pantalla poco comunes. La idea de Cris (cita textual, ortografía corregida) es la solución de fondo a aplicar después del MVP: "Lo único que me preocupa es que a futuro salgan celulares con más resolución y los pines queden chiquitos. ¿Vale la pena pasar de tamaño en px a % de ancho de imagen? Porque acá también puede solapar y generar problemas con celulares de pantalla cuadrada, y ¿qué pasa si la persona gira el celular poniéndolo en modo horizontal? Tal vez sería prudente establecer que el tamaño de la imagen maximizada debe ser un X% del ratio más corto que queda disponible luego de que el panel está estirado. Si no es complejo, hacerlo sería: una vez que se toma en cuenta el espacio que normalmente ocupa en su estado #1 el panel del pin, en base a ese % de tamaño de ancho de página en relación del aspect ratio del lado más corto que queda libre en mapa, o sea restándole el espacio que ocupa el panel con la info del lugar, de ese espacio que se ve de mapa en pantalla, asignarle un tanto %." Viabilidad confirmada por Claude: el proyecto ya tiene la pieza clave para esto en `getOpenAreaPx()` (js/poi-panel.js), usada hoy por panToPoiCenter — solo devuelve un lado (el que el panel afecta según orientación), haría falta extenderla para devolver también el otro lado libre y tomar el menor de los dos. Implica reescribir por completo qué significa el % del slider "Tamaño del edificio maximizado" (deja de ser % de la imagen full de 1024px, pasa a ser % del lado libre de mapa) — hay que recalibrar el valor default para no cambiar visualmente lo que ya funciona bien en mobile hoy, y además sumar recálculo en vivo si el usuario rota el celular con un pin ya maximizado abierto (hoy eso no se recalcula, gap ya existente que se vuelve más relevante con esta fórmula). Estimado en su momento: 3-4hs para el pin maximizado, +1-2hs si se extiende el mismo criterio al pin chico del mapa en reposo (sin el descuento del panel, ya que ahí no hay panel abierto).' },
];
